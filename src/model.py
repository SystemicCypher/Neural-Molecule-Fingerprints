...
def neural_graph_fingerprints(molecule_graph, layer_rep, radius, fingerprint_len, feature_size):
    inputs = layers.Input(shape=(feature_size,))
    all_outputs = []
    for i in range(1,radius):
        layer_rep, layer_fp = fingerprint_model_layer(molecule_graph, layer_rep, fingerprint_len, feature_size)
        all_outputs.append(layer_fp)
    final_fp = layers.merge(all_outputs, mode='sum')

    neural_fp = models.Model(input=inputs, output=[final_fp])
    neural_fp.compile(optimzer = optimizers.Adam(), loss='mean_squared_error')
    return neural_fp
